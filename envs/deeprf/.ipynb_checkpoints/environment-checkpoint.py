...
class DeepRFSLRINV_origin(gym.Env):
    def __init__(self, **kwargs):
        ...
        self.max_mag = kwargs.get('max_mag', 0.9512)
        self.max_ripple = kwargs.get('max_ripple', 0.01)
        refer_Mxy  = loadmat('./refer_Mz_inv.mat')
        a = np.array(refer_Mxy['Mz'], dtype = np.float32)
        Mz = np.repeat(a, repeats = 256, axis= 0)
        
        self.refer_Mz = torch.from_numpy(Mz)
    ...
    def step(self, actions):
        ...
        if done:
            Mz = Mt[:,0,:,2]
            refer_Mz = self.refer_Mz.to(Mz.device)
...
class DeepRFSLRINV_OC(gym.Env):
    def __init__(self, **kwargs):
        ...
        self.max_mag = kwargs.get('max_mag', 0.9512)
        self.max_ripple = kwargs.get('max_ripple', 0.01)
        refer_Mxy  = loadmat('./refer_Mt_inv2.mat')
        a = np.array(refer_Mxy['Mx'], dtype = np.float32)
        b = np.array(refer_Mxy['My'], dtype = np.float32)
        c = np.array(refer_Mxy['Mz'], dtype = np.float32)
        Mx = np.repeat(a, repeats = 50, axis= 0)
        My = np.repeat(b, repeats = 50, axis = 0)
        Mz = np.repeat(c, repeats = 50, axis=0)
    ...
    def step(self, actions):
        ...
        if done:
            ...
            refer_My = self.refer_My.to(My.device)
            refer_Mz = self.refer_Mz.to(Mz.device)
            error = torch.sum((Mx - refer_Mx).square(),dim=1) + torch.sum((My-refer_My).square(),dim=1) + torch.sum((Mz-refer_Mz).square(),dim=1)
            rewards += -1*error
        return Mt.permute(0, 2, 1, 3), rewards, done