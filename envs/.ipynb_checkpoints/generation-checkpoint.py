...
parser.add_argument('--du', type=float, default=5.12, help='duration of pulse in ms')
...
preset = loadmat('../demo/SLR_512_env_2.mat')
ref_pulse = torch.unsqueeze(torch.from_numpy(np.array(preset['RF'], dtype=np.float32)), dim=0).to(device)
...
with tf.Session() as sess:
    ...
    for it in range(args.max):
        ...
        with torch.no_grad():  # for inference only
            ...
            rew = total_rewards.detach().cpu().numpy()
